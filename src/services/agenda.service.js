...
export class AgendaService {
    ...
    cambiarTurnosPorDisponibilidadModificada(medico, disponibilidadAnterior, disponibilidadModificada){
        ...
        turnosTotales.forEach(turno => {
                if(turno.medico.id === medico.id && this.chequearDay(turno, disponibilidadAnterior) && turno.fechaHora > fechaActual && turno.estado === EstadoTurno.DISPONIBLE){
    ...
    chequearDay(turno, disponibilidad){
        if(turno.fechaHora.day == disponibilidad.obtenerIndiceDelDiaDeSemana(disponibilidad.diaSemana)){
                return true;
        }
        return false;
    }
